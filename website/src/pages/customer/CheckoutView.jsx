...
function CheckoutView() 
{
  ...
  const checkout = async () =>
  {
    ...
    await axios.post('http://localhost:3000/api/orders/create-order',
      ...
      .then(res => {
        navigate(`/success?oid=${res.data.transactionId}`, {replace: true});
      });
  };
    ...
    <main className="relative w-full h-full overflow-x-hidden gap-6 pt-10 p-4">
      ...
      <section className="flex flex-col border border-gray-400 rounded-md p-4 mb-4">
        ...
        <div className="flex flex-row items-start justify-left gap-7">
          ...
          <label htmlFor="flowbite-option" className="inline-flex flex-col items-start justify-center p-5 text-black border-2 border-yellow-400 rounded-2xl cursor-pointer dark:text-gray-500 dark:peer-checked:text-black dark:bg-yellow-50 dark:border-gray-300 dark:peer-checked:bg-yellow-400 peer-checked:border-black">
            <div className="block">
              <div className="w-full text-[15px] font-semibold">Cash on Delivery (COD)</div>
              <div className="w-full text-[13px]">Cash on Delivery</div>
            </div>
          </label>
          {/* Add other payment options here for design */}
          {/* <div className="flex flex-col">
            <label className="block text-black text-md mb-2" htmlFor="pay"> 
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-md mb-2" htmlFor="house-number">
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-md mb-2" htmlFor="street">
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-md mb-2" htmlFor="brgy">
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-md mb-2" htmlFor="province">
      ...
      <button onClick={checkout} className={`bg-green-700 hover:bg-green-400 text-white font-bold py-2 px-4 rounded-full inline-flex justify-center items-center w-full ${!formValid ? 'pointer-events-none opacity-50' : ''}`}>Order </button>
    </main>
  );
}
